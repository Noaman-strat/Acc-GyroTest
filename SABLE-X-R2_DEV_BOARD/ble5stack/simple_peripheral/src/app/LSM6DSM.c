...
void init_LSM6DSM_AccONLY(void){
    ...
    LSM6DSM_WriteReg(LSM6DSM_CTRL2_G,0x00); //Gyro powerdown
    //2 - Get Acc in high power mode
    LSM6DSM_WriteReg(LSM6DSM_CTRL6_C, 0x00);
    //3 - set one to the boot pit
    LSM6DSM_WriteReg(LSM6DSM_CTRL3_C,0x80); //reboot
    //4 - Wait 15 ms
    Task_sleep(500000);
    ...
    LSM6DSM_WriteReg(LSM6DSM_CTRL1_XL,(LSM6DSM_ODR_52HZ | LSM6DSM_FULLSCALE_2)); //free fall requires faster ODR, 12.5 Hz wont be enough
    LSM6DSM_WriteReg(LSM6DSM_INT1_CTRL,0x01); //Fifo threshold interrupt on Int_1
/*
    //Set mode to Low power
    //we are reading the current value as to not overwrite the other bits. dont know the default value
    RegVal = LSM6DSM_ReadReg(LSM6DSM_CTRL6_C);
    LSM6DSM_WriteReg(LSM6DSM_CTRL6_C, (0x00));     //disable high power mode, set XL user offset bit to 1
    ...
    //LSM6DSM_WriteReg(LSM6DSM_CTRL3_C, 0x44);
    LSM6DSM_WriteReg(LSM6DSM_CTRL3_C, 0x00);
...
uint8 LSM6DSM_GetAccAxesRaw(AxesRaw_t* buff) {
  ...
  LSM6DSM_ReadReg(LSM6DSM_WHO_AM_I_REG);
  //Task_sleep(2000); 
...
void configure_LSM_tiltInt(){
  ...
  LSM6DSM_WriteReg(LSM6DSM_CTRL1_XL,(LSM6DSM_ODR_26HZ | LSM6DSM_FULLSCALE_2)); //Absolute tilt requires 26 Hz
      
  LSM6DSM_WriteReg(LSM6DSM_CTRL10_C, 0x00);
  LSM6DSM_WriteReg(LSM6DSM_INT1_CTRL,0x00); //Clear INT1 control. No interrupts
  LSM6DSM_WriteReg(LSM6DSM_INT2_CTRL,0x00); //Clear INT2 control. No interrupts
  
  LSM6DSM_WriteReg(LSM6DSM_FUNC_CFG_ACCESS, 0xA0); //Enable access to Embedded functions registers
   Task_sleep(1000);
  LSM6DSM_WriteReg(A_WRIST_TILT_LAT, 0x0F);     //latency = 600 ms, 1 LSB = 40 ms
  LSM6DSM_WriteReg(A_WRIST_TILT_THS, 0x20);     //30 degree
  LSM6DSM_WriteReg(A_WRIST_TILT_Mask, 0xC0);    //X positive or X negative
  Task_sleep(1000);
  LSM6DSM_WriteReg(LSM6DSM_FUNC_CFG_ACCESS, 0x00); //Disable access to Embedded functions registers
  LSM6DSM_WriteReg(LSM6DSM_CTRL10_C, 0x84);     //Enable Wrist Tilt Detection
  
  LSM6DSM_WriteReg(LSM6DSM_DRDY_PULSE_CFG, 0x01); // AWT interrupt on INT2
  LSM6DSM_WriteReg(LSM6DSM_TAP_CFG1, 0x81); //enable latched interrupt
}
...
void LSM_Clear_tiltInt()
{
  ...
  char RegVal2 = LSM6DSM_ReadReg(LSM6DSM_FUNC_SRC2);
}
